...
function emitGraph(graph,block,ctx){
  ...
  /* among nodes whose dependencies are all satisfied, the one that appears
     first in graph.nodes executes next — that array order is the only record
     of sequence for anything not already pinned by a wire (unconnected
     islands, parallel branches), so this is what makes execution order match
     the diagram instead of incidental object-key iteration */
  const arrayIndex={}; nodes.forEach((n,i)=>arrayIndex[n.id]=i);
  const ready=nodes.filter(n=>deg[n.id]===0).map(n=>n.id);
  const order=[];
  while(ready.length){
...
/* ---- project file format: one .py file, not a separate .json --------
   The runnable program (GEN.full — what Deploy actually sends to a
   device) is completely unaffected; a real MCU has no use for a comment
   dump of the whole project appended to it, and never sees one. Only
   the saved-to-disk copy gets the graph embedded alongside it, as a
   clearly marked comment block a real Python interpreter just treats as
   nothing — so Open can read the exact same file Save just wrote and
   reconstruct the diagram exactly, with no separate project format
   anywhere. Every line is prefixed so ordinary line-based editing of the
   REST of the file (or a diff/git blame) never mistakes the block for
   code; the markers themselves can't appear inside the JSON they
   bracket, since JSON never emits a bare '#'. */
const MANIFEST_BEGIN='# === FLOWPY GRAPH — do not edit below by hand, regenerated on every change ===';
const MANIFEST_END  ='# === END FLOWPY GRAPH ===';
function manifestBlock(){
  const lines=JSON.stringify(P_,null,1).split('\n').map(l=>l?'# '+l:'#');
  return [MANIFEST_BEGIN,...lines,MANIFEST_END,''].join('\n');
}
function fileContent(){ return manifestBlock()+'\n'+GEN.full; }
function parseManifest(text){
  const i0=text.indexOf(MANIFEST_BEGIN), i1=text.indexOf(MANIFEST_END);
  if(i0===-1||i1===-1) throw new Error('not a FlowPy project file — no embedded graph found');
  const block=text.slice(i0+MANIFEST_BEGIN.length,i1);
